  ...
  return (
    <Divstyled>
      {category ? category.data.map((item, index) => {
        const isMatchingId = item._id === id;
        {isMatchingId && (document.title = item.name)}
        return isMatchingId && (
          <Divstyled key={index}>
              ...
              <Divstyled className='md:flex lg:flex block gap-2 '>
                ...
                <Divstyled className="lg:w-9/12 md:w-9/12">
                  <Divstyled className="category">
                    <DivstyledTitle className="md:text-[18px] lg:text-[20px] sm:text-[15px]" style={{ textTransform: 'capitalize' }}>{item.name}</DivstyledTitle>
                  </Divstyled>
