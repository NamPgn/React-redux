...
import { resgister, login, getUser, deleteAuth, editAuth, getAuth, importExcel, findCartByUser, editImageUser } from "../../api/user";
...
const userSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(deleteCartSlice.fulfilled, (state, action: any) => {
            const updatedCart = state.user.cart.filter(item => item.product._id !== action.payload.product);
